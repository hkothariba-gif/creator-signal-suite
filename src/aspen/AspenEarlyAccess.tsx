  ...
  renderVals(): any {
    ...
    return {
      ...
      setCompany: (e: React.ChangeEvent<HTMLInputElement>) =>
        this.setState({ company: e.target.value, error: "" }),
    ...
    return (
      <div className="aspen-scope min-h-screen bg-cream">
        <nav className="flex items-center justify-between max-w-[1240px] w-full mx-auto p-[20px_32px] box-border">
          ...
          <a href="/login" className="text-[15px] font-semibold">
            Log in
          </a>
        </nav>
        ...
        <div className="max-w-[1180px] mx-auto p-[40px_32px_90px] flex gap-[64px] flex-wrap items-start">
          <div className="flex-[1_1_420px] min-w-[300px]">
            ...
            <h1 className="font-heading font-extrabold text-[clamp(38px,4.6vw,60px)] tracking-[-0.035em] leading-[1.03] m-[14px_0_0] max-w-[520px] text-balance">
              Get on the list. Open your workspace first.
            </h1>
            <p className="text-[17.5px] text-muted leading-[1.55] m-[20px_0_0] max-w-[460px]">
              We're opening Aspen in cohorts this quarter. Tell us who you are and we'll bring you
              in with the next one.
            </p>

            <div className="flex flex-col gap-[18px] mt-[36px] max-w-[460px]">
              <div className="flex gap-[16px] items-start">
                <div className="w-[30px] h-[30px] rounded-[10px] bg-tint text-accent-ink grid place-items-center font-extrabold text-[13px] shrink-0">
                  1
                </div>
                <div>
                  <div className="font-bold text-[16px]">Onboarding with our team</div>
                  <div className="text-[14.5px] text-muted leading-[1.5] mt-[2px]">
                    We set up your first campaign with you, on a call.
                  </div>
                </div>
              </div>
              <div className="flex gap-[16px] items-start">
                ...
                <div>
                  <div className="font-bold text-[16px]">Launch pricing locked for 12 months</div>
                  <div className="text-[14.5px] text-muted leading-[1.5] mt-[2px]">
                    Whatever we launch at, you keep for a year.
                  </div>
                </div>
              </div>
              <div className="flex gap-[16px] items-start">
                ...
                <div>
                  <div className="font-bold text-[16px]">A say in what we build next</div>
                  <div className="text-[14.5px] text-muted leading-[1.5] mt-[2px]">
                    Early cohorts shape the roadmap — Instagram and TikTok included.
                  </div>
                </div>
              </div>
            </div>
            ...
            <img
              src="/aspen/workspace.webp"
              alt="The Aspen workspace — discovery, affiliates and ads in one place"
              className="w-full max-w-[460px] mt-[40px] block rounded-[18px]"
              loading="lazy"
            />
          </div>
          ...
          <div className="flex-[1_1_400px] min-w-[300px] max-w-[460px]">
            <div className="bg-surface border-[1.5px] border-border rounded-[24px] p-[32px] shadow-[0_24px_60px_rgba(23,20,30,0.07)]">
              {v.done ? (
                <>
                  <div>
                    <div className="w-[46px] h-[46px] rounded-[14px] bg-highlight grid place-items-center font-heading font-extrabold text-[22px]">
                      ✓
                    </div>
                    <h2 className="font-heading font-extrabold text-[26px] tracking-[-0.02em] m-[18px_0_0]">
                      You're on the list
                    </h2>
                    <p className="text-[15.5px] text-muted leading-[1.6] m-[12px_0_0]">
                      We'll email you when your cohort opens. Want a head start? Set up your first
                      campaign brief now — it takes two minutes.
                    </p>
                    <a
                      href="/onboarding"
                      className="block text-center mt-[22px] bg-accent text-cream text-[16px] font-bold p-[15px_0] rounded-[14px] ah48"
                    >
              ...
              {v.notDone ? (
                <>
                  <div>
                    <h2 className="font-heading font-extrabold text-[24px] tracking-[-0.02em] m-0">
                    ...
                    <div className="text-[12.5px] font-bold tracking-[0.06em] text-subtle m-[22px_0_10px]">
                      WHAT DO YOU SELL?
                    </div>
                    <div className="flex gap-[8px] flex-wrap">
                      {(v.segments || []).map((s: any, $index: number) => (
                        <React.Fragment key={$index}>
                          <button
                            onClick={s.select}
                            className="font-sans text-[13.5px] font-semibold p-[10px_15px] rounded-[11px] cursor-pointer"
                            style={{ border: `1.5px solid ${s.bd}`, background: s.bg, color: s.fg }}
                          >
                            {s.label}
                          </button>
                        </React.Fragment>
                    ...
                    <label
                      htmlFor="brief"
                      className="block text-[12.5px] font-bold tracking-[0.06em] text-subtle m-[22px_0_8px]"
                    >
                      WHO ARE YOU TRYING TO REACH?{" "}
                      <span className="font-semibold tracking-[0]">(optional)</span>
                    </label>
                    <textarea
                      id="brief"
                      value={v.brief}
                      onChange={v.setBrief}
                      placeholder="e.g. HR leaders at 200–2,000 person companies in the US"
                      rows={3}
                      className="w-full box-border p-[14px_16px] rounded-[13px] border-[1.5px] border-border bg-cream font-sans text-[15px] leading-[1.5] text-dark outline-none resize-y"
                    ></textarea>

                    <button
                      onClick={v.submit}
                      className="w-full mt-[24px] h-[54px] border-0 rounded-[14px] bg-accent text-cream font-sans text-[16.5px] font-bold cursor-pointer ah49"
                    >
                      {v.buttonLabel}
                    </button>
                    {v.error ? (
                      <>
                        <div className="mt-[14px] text-[14px] font-semibold text-accent-ink">
                          {v.error}
                        </div>
                      </>
                    ) : null}
                    <p className="text-[13px] text-subtle m-[16px_0_0] leading-[1.5]">
                      No credit card, no contract. Enterprise team?{" "}
