...
export default class AspenOnboarding extends React.Component<AspenOnboardingProps, any> {
  state: any = {
    ...
    picked: { youtube: true, reddit: true, x: true, linkedin: false },
    busy: false,
  };
    ...
    return picked.filter((f) => {
      if (f.size > max) {
        this.props?.onFileError?.(
          `${f.name} is too large (max ${Math.round(max / (1024 * 1024))}MB)`,
        );
        return false;
      }
      return true;
  ...
  platformMeta = [
    { key: "youtube", label: "YouTube", glyph: "▶", color: "#F03", sub: "Long form · high intent" },
    {
      ...
      glyph: "r/",
      color: "#FF4500",
      sub: "Community trust · cheap clicks",
    },
  ...
  renderVals(): any {
    ...
    return {
      ...
      docs: s.docs.map((f: File, i: number) => ({
        name: f.name,
        remove: () =>
          this.setState((st: any) => ({ docs: st.docs.filter((_: File, j: number) => j !== i) })),
      })),
      platforms: this.platformMeta.map((p) => {
        ...
        return {
          ...
          toggle: () =>
            this.setState((st: any) => ({ picked: { ...st.picked, [p.key]: !st.picked[p.key] } })),
        };
      }),
      setBrief: (e: React.ChangeEvent<HTMLTextAreaElement>) =>
        this.setState({ brief: e.target.value }),
      setNotes: (e: React.ChangeEvent<HTMLTextAreaElement>) =>
        this.setState({ notes: e.target.value }),
      setAge: (e: React.ChangeEvent<HTMLSelectElement>) => this.setState({ age: e.target.value }),
      setSeniority: (e: React.ChangeEvent<HTMLSelectElement>) =>
        this.setState({ seniority: e.target.value }),
      ...
      next: () => {
        if (this.state.brief.trim()) this.setState({ step: 2 });
      },
      ...
      <div className="aspen-scope min-h-screen bg-cream">
        <div className="h-[4px] bg-border">
          <div
            ...
            style={{ transition: "width 0.5s ease", width: v.progress }}
          ></div>
        </div>
        ...
        <div className="max-w-[760px] mx-auto p-[24px_24px_100px]">
          {v.isStep1 ? (
            <>
              <div>
                <h1 className="font-heading font-extrabold text-[clamp(32px,4vw,44px)] tracking-[-0.03em] leading-[1.05] m-0">
                  Tell us about your product
                </h1>
                <p className="text-[16.5px] text-muted leading-[1.6] m-[14px_0_0] max-w-[560px]">
                  Everything here becomes your first campaign — creator matching, audience scoring,
                  and the ads you build all read from it. Write freely, or attach a doc.
                </p>

                <textarea
                  value={v.brief}
                  onChange={v.setBrief}
                  placeholder="Describe your product, your brand, and who you're trying to reach — as much detail as you like…"
                  rows={7}
                  className="w-full box-border mt-[28px] p-[20px] rounded-[18px] border-[1.5px] border-border bg-surface text-[16px] leading-[1.6] text-dark outline-none resize-y"
                ></textarea>
                ...
                <div className="mt-[16px] border-[1.5px] border-dashed border-sand-line rounded-[16px] bg-surface p-[18px_20px] flex items-center justify-between gap-[16px] flex-wrap">
                  <div>
                    ...
                    <div className="text-[13.5px] text-subtle mt-[2px]">
                      PDF, TXT or MD — optional. We pull real excerpts into your ad corpus, so
                      drafts quote you, not us.
                    </div>
                  </div>
                  <label
                    htmlFor="aspen-brand-docs"
                    className="inline-block border-[1.5px] border-dark bg-transparent font-sans text-[14px] font-bold p-[11px_18px] rounded-[12px] cursor-pointer whitespace-nowrap ah50"
                  >
                    + Add file
                  </label>
                  <input
                ...
                {(v.docs || []).map((d: any, $index: number) => (
                  <React.Fragment key={$index}>
                    <div className="mt-[10px] bg-surface border-[1.5px] border-border rounded-[13px] p-[13px_16px] flex items-center justify-between gap-[12px]">
                      ...
                      <button
                        onClick={d.remove}
                        className="border-0 bg-transparent font-sans text-[13px] font-bold text-subtle cursor-pointer ah51"
                      >
                        Remove
                      </button>
                    </div>
                ...
                <div className="flex justify-center mt-[40px]">
                  <button
                    onClick={v.next}
                    style={{ opacity: v.nextOpacity }}
                    className="border-0 bg-accent text-cream font-sans text-[16.5px] font-bold p-[16px_40px] rounded-[14px] cursor-pointer ah52"
                  >
                    Continue →
                  </button>
                </div>
          ...
          {v.isStep2 ? (
            <>
              <div>
                <button
                  onClick={v.back}
                  className="border-0 bg-transparent font-sans text-[13.5px] font-bold text-subtle cursor-pointer p-0 mb-[20px] ah53"
                >
                  ← Back
                </button>
                <h1 className="font-heading font-extrabold text-[clamp(32px,4vw,44px)] tracking-[-0.03em] leading-[1.05] m-0">
                ...
                <p className="text-[16.5px] text-muted leading-[1.6] m-[14px_0_0] max-w-[560px]">
                  We score every creator's audience against this before you spend a cent.
                </p>
                ...
                <div className="grid grid-cols-[repeat(auto-fit,minmax(190px,1fr))] gap-[14px] mt-[28px]">
                  <div>
                    ...
                    <select
                      ...
                      value={v.age}
                      onChange={v.setAge}
                      className="w-full h-[48px] p-[0_14px] rounded-[13px] border-[1.5px] border-border bg-surface text-[15px] text-dark outline-none"
                    >
                ...
                <label
                  htmlFor="notes"
                  className="block text-[12.5px] font-bold tracking-[0.06em] text-subtle m-[24px_0_8px]"
                >
                ...
                <textarea
                  ...
                  value={v.notes}
                  onChange={v.setNotes}
                  placeholder="e.g. People ops leads who already run a tool they complain about"
                  rows={3}
                  className="w-full box-border p-[14px_16px] rounded-[15px] border-[1.5px] border-border bg-surface text-[15px] leading-[1.55] text-dark outline-none resize-y"
                ></textarea>

                <div className="mt-[32px]">
                  <div className="text-[12.5px] font-bold tracking-[0.06em] text-subtle">
                    WHERE DO YOUR BUYERS HANG OUT?
                  </div>
                  <div className="text-[13.5px] text-subtle mt-[4px]">
                    Pick every platform we should scout creators on.
                  </div>
                  <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-[12px] mt-[16px]">
                    {(v.platforms || []).map((p: any, $index: number) => (
                      <React.Fragment key={$index}>
                        <button
                          onClick={p.toggle}
                          className="text-left cursor-pointer p-[18px] rounded-[18px] font-sans"
                          style={{ border: `1.5px solid ${p.bd}`, background: p.bg }}
                        >
                          <div className="flex items-center justify-between">
                            <span className="inline-flex items-center gap-[9px]">
                              <span
                                className="w-[26px] h-[26px] rounded-[8px] text-surface grid place-items-center font-extrabold text-[11px]"
                ...
                <div className="mt-[28px] border-[1.5px] border-dashed border-sand-line rounded-[16px] bg-surface p-[18px_20px] flex items-center justify-between gap-[16px] flex-wrap">
                  <div>
                    ...
                    <div className="text-[13.5px] text-subtle mt-[2px]">
                      Optional. Upload a CSV of your best customers and we'll find creators whose
                      audiences look like them.
                    </div>
                  </div>
                  <label
                    htmlFor="aspen-lookalike-sheet"
                    className="inline-block border-[1.5px] border-dark bg-transparent font-sans text-[14px] font-bold p-[11px_18px] rounded-[12px] cursor-pointer whitespace-nowrap ah54"
                  >
                    {v.sheetLabel}
                  </label>
                  <input
                    id="aspen-lookalike-sheet"
                    type="file"
                ...
                <button
                  onClick={v.finish}
                  className="w-full mt-[36px] border-0 bg-accent text-cream font-sans text-[17px] font-bold p-[18px_0] rounded-[15px] cursor-pointer ah55"
                >
                  {v.finishLabel}
                </button>
                <p className="text-center text-[13.5px] text-subtle leading-[1.55] m-[14px_0_0]">
          ...
          {v.isDone ? (
            <>
              <div className="text-center p-[40px_0]">
                <div className="w-[52px] h-[52px] rounded-[16px] bg-highlight grid place-items-center font-heading font-extrabold text-[24px] mx-auto">
                  ✓
                </div>
                <h1 className="font-heading font-extrabold text-[clamp(30px,3.6vw,42px)] tracking-[-0.03em] leading-[1.05] m-[22px_0_0]">
                  Your first campaign is ready
                </h1>
                <p className="text-[16.5px] text-muted leading-[1.6] m-[14px_auto_0] max-w-[460px]">
                  We're scoring creators against your brief now. When your cohort opens, this is the
                  first thing you'll see.
                </p>
                <img
                  src="/aspen/signal.webp"
                  alt="Clay world map of creator districts across four platforms"
                  className="w-full max-w-[520px] m-[32px_auto_0] block rounded-[18px]"
                  loading="lazy"
                />
                <div className="flex justify-center gap-[12px] mt-[32px] flex-wrap">
                  <a
                    href="/"
                    className="bg-dark text-cream text-[16px] font-bold p-[15px_28px] rounded-[14px] ah56"
                  >
                    Back to Aspen
                  </a>
                  <a
                    href="/tour"
                    className="border-[2px] border-dark text-[16px] font-bold p-[13px_26px] rounded-[14px] ah57"
                  >
