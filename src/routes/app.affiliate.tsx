  ...
  return (
    <AppShell title="Affiliate & Payouts">
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={s.label} className="bg-[#0C1222] border border-white/[0.07] rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-[#F0F4FF]">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-[#0C1222] border border-white/[0.07] rounded-lg overflow-hidden">
          ...
          <thead>
            <tr className="border-b border-white/[0.07]">
              {["Campaign Name", "Affiliate URL", "Clicks", "Conversions", "Revenue", "Created", ""].map((h) => (
                <th key={h} className="text-left px-4 py-3 text-xs text-[#8892A4] font-medium">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {links.map((link) => (
              <tr key={link.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                <td className="px-4 py-3 text-[#F0F4FF] font-medium">{link.name}</td>
                <td className="px-4 py-3">
                  <span className="text-[#00D97E] font-mono text-xs truncate max-w-[180px] block">{link.url}</span>
                </td>
                <td className="px-4 py-3 text-[#8892A4]">{link.clicks.toLocaleString()}</td>
                <td className="px-4 py-3 text-[#8892A4]">{link.conversions.toLocaleString()}</td>
                <td className="px-4 py-3 text-[#00D97E] font-semibold">{link.revenue}</td>
                <td className="px-4 py-3 text-[#8892A4] text-xs">{link.created}</td>
                <td className="px-4 py-3">
                  <button
        ...
        {links.length === 0 && (
          <div className="text-center py-12 text-[#8892A4]">No affiliate links yet. Generate your first link above.</div>
        )}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-[#0C1222] border border-white/[0.1] rounded-xl p-6 w-full max-w-md">
            <h3 className="text-lg font-bold text-[#F0F4FF] mb-4">Generate New Affiliate Link</h3>
            <label className="block text-xs text-[#8892A4] mb-1">Campaign / Creator Name</label>
            <input
              value={newName}
