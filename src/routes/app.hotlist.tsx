...
function HotlistPage() {
  ...
  const filtered = useMemo(
    () =>
      filter === "All"
        ? rows
        : rows.filter((r) => (r.platform ?? "").toLowerCase() === filter.toLowerCase()),
    [rows, filter],
  );
  ...
  return (
    <AppShell title="Hotlist CRM">
      <p className="text-[#8892A4] mb-4">Your shortlisted creators, organized by stage</p>
        ...
        {PLATFORM_FILTERS.map((p) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${
              filter === p ? "bg-[#00D97E] text-[#05080F]" : "bg-white/[0.05] text-[#8892A4] hover:text-white"
            }`}
          >
            {p}
          </button>
