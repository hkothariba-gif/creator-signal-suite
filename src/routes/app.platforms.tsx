...
const CARDS: {
...
}[] = [
  {
    ...
    tag: "Video",
    body: "Channel emails, topic clusters and audience fit all read from the YouTube Data API.",
  },
  {
    key: "reddit",
    glyph: "r/",
    ...
    tag: "Ads",
    body: "Subreddit mapping, sentiment and Promoted Posts, seeded by your best-performing videos.",
  },
  {
    key: "x",
    glyph: "X",
    ...
    tag: "Social",
    body: "Find open-DM creators, run sequences, then whitelist the posts that land as X Ads.",
  },
  {
    key: "linkedin",
    glyph: "in",
    ...
    tag: "B2B",
    body: "Match voices to your buying committee and track which posts influence pipeline.",
  },
];
    ...
    <div className="aspen-scope max-w-[1020px]">
      <div className="grid grid-cols-[repeat(auto-fit,minmax(330px,1fr))] gap-[16px]">
        {CARDS.map((p) => {
          ...
          return (
            <div
              key={p.key}
              className="bg-surface border-[1.5px] border-border rounded-[20px] overflow-hidden"
            >
              <div
                className="flex items-center gap-[12px] p-[18px_20px]"
                style={{ background: p.bandBg }}
              >
                <span
                  className="w-[34px] h-[34px] rounded-[11px] text-surface grid place-items-center font-extrabold text-[13px] shrink-0"
                ...
                <span className="text-[11px] font-bold p-[5px_10px] rounded-[8px] bg-surface text-muted">
                  {p.tag}
                </span>
              </div>
      ...
      <div className="bg-surface border-[1.5px] border-border rounded-[20px] p-[20px_22px] mb-[16px]">
        <p className="text-[13.5px] text-muted leading-[1.55] m-0">
          Integration keys are configured on the server by the platform team. They are never stored
          in the browser. Panels that depend on an integration show “{WAITING_COPY}” until it is
          configured.
        </p>
      </div>
